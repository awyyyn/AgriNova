...
export default function PestsFound() {
	...
	const fetchHistory = async (
		...
		try {
			...
			const res = await fetch(
				`${process.env.EXPO_PUBLIC_API_URL}/plant/list?page=${pageNumber}&limit=${PAGE_SIZE}&hasPest=true`,
				{
					headers: {
			...
			const items = data.filter((item) => !!item.diagnosis);

			replace ? setItems(items) : addItems(items);
		} catch (err) {
	...
	const renderItem = ({ item }: { item: PlantAnalysis }) => (
		<TouchableOpacity
			...
			// }
			className="flex-row gap-3 bg-white p-3 rounded-xl shadow-sm mb-3 h-auto">
			{/* Image */}
			...
			<View className="flex-1 justify-between">
				<VStack className="flex-1  justify-between">
					<View>
						...
						<Text className="text-2xl  ">{item.diagnosis?.name}</Text>
					</View>

					<VStack className="items-">
						<Text className="text-sm text-gray-400">Pest Found on:</Text>
						<Text className="text-md text-gray-600">
							{format(new Date(item.createdAt), "MMM dd, yyyy").toString()} at{" "}
							{format(new Date(item.createdAt), "MM:hh a")}
						</Text>
					</VStack>
				</VStack>
			</View>
